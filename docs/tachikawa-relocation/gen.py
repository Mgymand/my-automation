...
# ---------- 定性比較（共通） ----------
QUAL = [
 ('初期資金・資金拘束','小さい。保証金・礼金・保証料の計{RENT_INIT}＋内装等{COMMON}＝<b>{RENT_TOT}</b>。解約時に保証金の8割が戻る','大きい。物件代金＋諸費用{BUY_TOKA}＋内装等{COMMON}＝<b>{BUY_TOT}</b>を一括拘束'),
...
SPECIFIC = {
 'A': dict(
  verdict='購入は「10年純資産」では有利だが、キャッシュ回収は20年超。<b>用途制限（事務所利用不可）が解消できなければ検討対象外</b>',
  verdict_cls='neg',
  ...
  strong=['<b>新耐震基準</b>（昭和58年4月築）。融資・保険・将来売却で不利になりにくい',
          ...
          '専有56.14㎡は<b>目標条件40㎡を+40%上回る</b>。同じ賃料24万円の枠でブース2台＋待合・カウンセリング室まで確保できる',
  ...
  weak=['<b>事務所利用不可</b>（上記警告）。ペット不可と併記され、居住専用色が強い',
        ...
        '単純回収年数 <b>20.5年</b>。10年の勘定は「最後に物件が残る」ことで成立しており、'
        '売却できなければ回収できない',
        '内装工事費は元モデルの200万円をそのまま使用。坪30万円で引き直すと約509万円となり、'
        '初期支出は両シナリオとも約309万円増える（<b>共通費のため賃貸／購入の優劣は変わらない</b>）'],
 ),
 'B': dict(
  verdict='<b>投資効率は明確に良好</b>（単純回収9.6年・実質利回り10.4%）。旧耐震と管理規約の用途制限がクリアできるかが唯一の関門',
  verdict_cls='pos',
  ...
  strong=['<b>㎡単価 412,071円</b>。A物件の約1/2で、駅距離（徒歩7分）はほぼ同等。'
          ...
          '<b>単純回収年数 9.6年・実質利回り 10.4%</b>（家賃24万円回避ベース）。'
          '10年でキャッシュベースでも賃貸を上回る（+378万円）',
          '<b>商業地域・防火地域</b>。店舗用途に対する行政上の制約が最も緩い区分',
          '<b>賃貸が有利に反転する分岐点は「余剰資金を年12.6%で回せるか」</b>。'
          'Aの5.43%より高く、購入判断が下振れに強い',
          '専有48.05㎡は<b>目標条件40㎡を+20%上回る</b>。新規リフォーム済（水回り3点・建具・給湯器・'
          'インターホン交換）で躯体側の追加投資も当面小さい'],
  weak=['<b>1981年5月築＝旧耐震の可能性が高い</b>（新耐震は建築確認1981年6月1日以降）。'
        ...
        '専有48.05㎡は条件（40㎡）は満たすがAより8.1㎡狭い。ブース2台＋待合は組めるがバックヤードが窮屈',
        '<b>固都税は未開示のため年69,432円と推計</b>（A物件の㎡単価で按分）。実額で要検証',
        '内装工事費は元モデルの200万円をそのまま使用。坪30万円で引き直すと約436万円（共通費のため優劣は不変）',
...
<tr><td>礼金・保証会社</td><td class="n">{y(r['reikin']+r['hoshougaisha'])}</td><td class="n">—</td><td>各1ヶ月</td></tr>
<tr><td>仲介手数料</td><td class="n">0</td><td class="n">0</td><td><b>自社仲介のため不要。</b>第三者仲介なら売買{man(d['chukai_ref'])}・賃貸{man(RENT_BASE*1.1)}</td></tr>
...
<tr class="tot"><td>物件取得・契約費用 計</td><td class="n">{y(r['shokihiyou'])}</td><td class="n">{y(d['toka'])}</td><td>諸費用は価格の{pc(d['shohiyou']/d['price'],1)}（仲介手数料ゼロ前提）</td></tr>
...
<h2>5. もう1件の候補との比較</h2>
{ab_table}
<p class="note"><b>緑の網掛け＝その項目で有利な方</b>（差が3%未満は実質互角として無色）。両物件とも立川駅徒歩6〜7分・空室即引渡し・ペット不可の分譲マンション1室で、<b>目標条件（40㎡以上・賃料24万円以下）は面積・立地とも充足</b>。営業収支の前提は完全に同一で、違いは価格・面積・築年・保有コストのみ。<b>この表の判断は「サロン営業が管理規約上できる」ことが前提</b>で、用途可否が確認できない物件は金額の優劣以前に候補から外れる。</p>

<div class="pb"></div>
<div class="hdr"><div><h1>{p['short']}　／　10年比較・感応度・売買のメリット</h1>
<p class="sub">物件条件は<b>40㎡以上・賃料24万円以下</b>（中野・立川とも同条件）。賃貸は保証金6ヶ月・礼金1・更新料2年毎1ヶ月。売買は融資を使わない現金一括（返済・金利・保証料・抵当権設定費用は発生しない）。<b>売買・賃貸とも仲介手数料は自社仲介のため発生しない前提。</b></p></div><div class="badge">物件{key}</div></div>
...
<h2>7. 感応度分析</h2>
<p class="note" style="margin-bottom:1mm"><b>①「回避できる家賃」の前提を動かす</b>──月24万円は中野・立川とも<b>上限</b>として確認済み。実勢がこれを下回るほど購入の妙味は薄れるため、下振れ側を確認しておく。</p>
{sens_rent}
...
<p class="note" style="margin-top:1.5mm">次アクション：①管理組合へ<b>店舗（美容所）用途の可否を書面照会</b>　②<b>重要事項調査報告書</b>で修繕積立金残高・長期修繕計画・滞納・値上げ予定を確認　③保健所に<b>美容所登録の構造設備基準</b>を事前相談　④<b>固定資産税評価証明</b>を取得し、登録免許税・不動産取得税の推計を実額に置換　⑤売買契約書または評価額按分で<b>建物比率</b>を確定させ、償却額を確定</p>
</div>

<div class="foot">前提の出所：営業収支＝「直営仕入れ中野」シート／賃貸条件＝前提条件シート（保証金6ヶ月・礼金1・保証料1・更新料2年毎1ヶ月・償却20%・仲介0）／仲介手数料は売買・賃貸とも自社仲介のためゼロ。購入諸費用＝登録免許税＝評価額×1.7%、不動産取得税＝評価額×2.5%、司法書士10万円、印紙1万円。固定資産税評価額は価格の50%と推計。減価償却は建物比率40%・中古簡便法{d['shou_nen']}年。実効法人税率33%。<b>推計値は物件ごとに評価証明・管理規約・重要事項調査報告書で要検証。</b></div>
"""
